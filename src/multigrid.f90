...
! #endif
    ...
    subroutine multigrid_no_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        ...
        enddo
        if(myrank.eq.0) print '(a,i4,a)','[MG] ========================== Total ',cyc,' V-cycles end =========================='
    ...
    subroutine multigrid_single_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        ...
        integer(kind=4)                             :: ddtype_temp1, ddtype_send, ddtype_gatherv
        integer(kind=4)                             :: ddtype_scatterv, ddtype_recv
        ...
        integer(kind=4), allocatable, dimension(:)  :: cnt_scatterv
        integer(kind=4), allocatable, dimension(:)  :: disps_scatterv
        ...
        allocate(cnt_gatherv(0:nprocs-1))
        allocate(disps_gatherv(0:nprocs-1))
        ...
        lb = 0
        extent = r8size
        call MPI_Type_create_resized(ddtype_temp1, lb, extent, ddtype_send, ierr)
        call MPI_Type_commit(ddtype_send, ierr)
        ...
        do i = 0, nprocs-1
            cnt_gatherv(i) = 1
            disps_gatherv(i) = nx * cart_coord(0,i)  &
                             + ny * cart_coord(1,i) * (nx_aggr + 2)  &
                             + nz * cart_coord(2,i) * (nx_aggr + 2) * (ny_aggr + 2)
        enddo

        ! For MPI_Scatterv
        allocate(cnt_scatterv(0:nprocs-1))
        allocate(disps_scatterv(0:nprocs-1))
        ...
        lb = 0
        extent = r8size
        call MPI_Type_create_resized(ddtype_temp1, lb, extent, ddtype_recv, ierr)
        call MPI_Type_commit(ddtype_recv, ierr)

        do i = 0, nprocs-1
            cnt_scatterv(i) = 1
            disps_scatterv(i) = nx * cart_coord(0,i)  &
                             + ny * cart_coord(1,i) * (nx_aggr + 2)  &
                             + nz * cart_coord(2,i) * (nx_aggr + 2) * (ny_aggr + 2)
        enddo
        !============= for communication  ============
...
#ifdef MPI_INPLACE
            if(myrank.eq.0) then
                call MPI_Gatherv(MPI_IN_PLACE, 0, ddtype_send, mg_sdm(lv_aggregation)%b, cnt_gatherv, disps_gatherv, ddtype_gatherv, 0, MPI_COMM_WORLD, ierr)
            else
                call MPI_Gatherv(mg_sdm(lv_aggregation)%b, 1, ddtype_send, mg_sdm(lv_aggregation)%b, cnt_gatherv, disps_gatherv, ddtype_gatherv, 0, MPI_COMM_WORLD, ierr)
            endif
#else
            allocate(tmp(0:size(mg_sdm(lv_aggregation)%b, 1)-1,0:size(mg_sdm(lv_aggregation)%b,2)-1,0:size(mg_sdm(lv_aggregation)%b,3)-1))
            call MPI_Gatherv(mg_sdm(lv_aggregation)%b, 1, ddtype_send, tmp, cnt_gatherv, disps_gatherv, ddtype_gatherv, 0, MPI_COMM_WORLD, ierr)
...
#ifdef MPI_INPLACE
            if(myrank.eq.0) then
                call MPI_Scatterv(mg_sdm(lv_aggregation)%x, cnt_scatterv, disps_scatterv, ddtype_scatterv, MPI_IN_PLACE, 0, ddtype_recv, 0, MPI_COMM_WORLD, ierr)
            else
                call MPI_Scatterv(mg_sdm(lv_aggregation)%x, cnt_scatterv, disps_scatterv, ddtype_scatterv, mg_sdm(lv_aggregation)%x, 1, ddtype_recv, 0, MPI_COMM_WORLD, ierr)
            endif
#else
            call MPI_Scatterv(mg_sdm(lv_aggregation)%x, cnt_scatterv, disps_scatterv, ddtype_scatterv, tmp, 1, ddtype_recv, 0, MPI_COMM_WORLD, ierr)
        ...
        deallocate( cnt_gatherv )
        deallocate( disps_gatherv )
        deallocate( cnt_scatterv )
        deallocate( disps_scatterv )
    ...
    subroutine multigrid_adaptive_aggregation_vcycle_solver(sol, a_poisson, rhs, sdm, maxiteration, tolerance, omega_sor)
        ...
        use mpi
        use mpi_topology, only  : myrank, nprocs, mpi_world_cart, cart_comm_1d, comm_1d_x, comm_1d_y, comm_1d_z
        ...
        integer(kind=4)                             :: ddtype_scatterv_x, ddtype_recv_x
        integer(kind=4)                             :: sizes(0:2), subsizes(0:2), starts(0:2)
        integer(kind=4)                             :: r8size
        integer(kind=4)                             :: nx_aggr, ny_aggr, nz_aggr
        integer(kind=4)                             :: nx, ny, nz
        ...
        !============= for communication  ============
        allocate(cnt_gatherv_x(0:comm_1d_x%nprocs-1))
        allocate(disps_gatherv_x(0:comm_1d_x%nprocs-1))
        ...
        lb = 0
        extent = r8size
        call MPI_Type_create_resized(ddtype_temp1, lb, extent, ddtype_send_x, ierr)
        call MPI_Type_commit(ddtype_send_x, ierr)
        ...
        print *, myrank, lv_aggr_max_ptr, lv_aggr_med_ptr, lv_aggr_min_ptr, max, med, min
        ...
        do cyc = 1, n_vcycles
            ...
            if(comm_min_ptr%myrank.eq.0) then
                ...
                do l = lv_aggr_min_ptr, lv_aggr_med_ptr-1
                    mg_sdm(l)%x = 0.0d0
                    call rbgs_iterator_poisson_matrix(mg_sdm(l)%x, mg_a_poisson(l), mg_sdm(l)%b, mg_sdm(l), maxiteration, omega_sor, mg_sdm(l)%is_aggregated)
                    call multigrid_residual(mg_sdm(l)%r, mg_a_poisson(l), mg_sdm(l)%x, mg_sdm(l)%b, mg_sdm(l), mg_sdm(l)%is_aggregated)
                    call multigrid_restriction(mg_sdm(l+1)%b, mg_sdm(l)%r, mg_sdm(l+1), mg_sdm(l),l)
                ...
                if(comm_med_ptr%myrank.eq.0) then
                    ...
                    do l = lv_aggr_med_ptr, lv_aggr_max_ptr-1
                        mg_sdm(l)%x = 0.0d0
                        call rbgs_iterator_poisson_matrix(mg_sdm(l)%x, mg_a_poisson(l), mg_sdm(l)%b, mg_sdm(l), maxiteration, omega_sor, mg_sdm(l)%is_aggregated)
                        call multigrid_residual(mg_sdm(l)%r, mg_a_poisson(l), mg_sdm(l)%x, mg_sdm(l)%b, mg_sdm(l), mg_sdm(l)%is_aggregated)
                        call multigrid_restriction(mg_sdm(l+1)%b, mg_sdm(l)%r, mg_sdm(l+1), mg_sdm(l),l)
                    ...
                    if(comm_max_ptr%myrank.eq.0) then
                        ...
                        do l = lv_aggr_max_ptr, n_levels-1
                            mg_sdm(l)%x = 0.0d0
                            call rbgs_iterator_poisson_matrix(mg_sdm(l)%x, mg_a_poisson(l), mg_sdm(l)%b, mg_sdm(l), maxiteration, omega_sor, mg_sdm(l)%is_aggregated)
                            call multigrid_residual(mg_sdm(l)%r, mg_a_poisson(l), mg_sdm(l)%x, mg_sdm(l)%b, mg_sdm(l), mg_sdm(l)%is_aggregated)
                            call multigrid_restriction(mg_sdm(l+1)%b, mg_sdm(l)%r, mg_sdm(l+1), mg_sdm(l),l)
...
#endif
                        ! Update the ghostcells in x-direction using derived datatypes and subcommunicator
                        do l = n_levels-1, lv_aggr_max_ptr, -1
                            call multigrid_prolongation_linear_on_nonuniform_grid(mg_sdm(l)%r, mg_sdm(l+1)%x, mg_sdm(l), mg_sdm(l+1),l)
                            mg_sdm(l)%x = mg_sdm(l)%x + mg_sdm(l)%r
                            call rbgs_iterator_poisson_matrix(mg_sdm(l)%x, mg_a_poisson(l), mg_sdm(l)%b, mg_sdm(l), maxiteration, omega_sor, mg_sdm(l)%is_aggregated)
                        enddo
        ...
        deallocate( rsd )
        deallocate(cnt_gatherv_x  )
        deallocate(disps_gatherv_x)

module cg_poisson_matrix
    ...
    ! #endif
        ...
        call vv_dot_3d_matrix(rsd0tol, rsd, rsd, dm%nx, dm%ny, dm%nz, is_aggregated)
        ! if(myrank.eq.0) print '(a,e15.7)', '   [CG] Conjugate gradient iterator begins. Initial residual = ', rsd0tol
...
#ifdef DEBUG
            if ((mod(iter,10).EQ.0).AND.(myrank.eq.0)) then
                ! print 101, sqrt(temp2), sqrt(temp2/rsd0tol), sqrt(rsd0tol), iter
                ! 101   format('   [CG] mse: ',e14.6,x,', r_mse: ',e14.6,x,', rsd0: ',e14.6,' at ',i5,' iterations.')
            endif
#endif
    ...
    ! #endif
        ...
        enddo
        if(myrank.eq.0) print '(a,i5,a,e15.7,a,e15.7)','   [CG] CG iterator completed with total iteration = ',iter,', mse = ',sqrt(temp2),', r_mse = ',sqrt(temp2/rsd0tol)

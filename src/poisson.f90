...
!    -(2*a + 4*a^2*x^2)*exp(a*(x^2 - 1))/(1-exp(-a))
    ...
    ! call multigrid_vcycle_solver(s_domain%x, a_poisson, s_domain%b, s_domain, maxiteration, tolerance, number_of_vcycles, number_of_levels, aggregation_method, aggregation_level, omega_sor)
    ...
    ! call geometry_boundary_value_calculate(s_domain%x, s_domain, g_domain, comm_boundary)
    ! call multigrid_vcycle_solver(s_domain%x, a_poisson, s_domain%b, s_domain, maxiteration, tolerance, number_of_vcycles, number_of_levels)
    ! rms = 0.0d0
    ! rms_local = 0.0d0
    ! do k=1, s_domain%nz
    !     do j=1, s_domain%ny
